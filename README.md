...
**Existing filters** decides what a run does when a destination user already has saved filters:

- **Add alongside** - the default, and what previous versions always did: the copied filters are appended and the user's own stay untouched.
- **Overwrite** - the destination's saved filters of the copied kind (Problems, Hosts, Latest data) are replaced by the source set.
- **Skip existing** - only the filters the user does not already have are copied, matched by filter name within the same section.

The list shows a **Last run** column for every assignment: the time of the last run followed by **(updated: N)** - how many users' filters actually changed - or **(no changes)** when a rerun found nothing to do. An assignment that has never run reads **Never**.
...
| Existing filters policy - Add alongside / Overwrite / Skip existing | ✅ |
| Last run overview per assignment | ✅ |
